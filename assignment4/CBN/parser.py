...
TOKEN_REGEX = re.compile(
    r'\s*(-?\d+|\w+|\(|\)|(\*)|\?|\=|\+|\-|\%|\/|\>|\<|(flk|lam|app|prim|error|if|pair|#t|#f|#u|snd|fst|not|and|or|!=|>=|<=)(\b|$))')
...
#        logger.debug('m: {}'.format(m))
        if m is None:
            n = TOKEN_REGEX1.match(source, start)
            if n is None: raise ValueError('LEXICAL_ERROR')
            else : yield n.group(0).strip()
