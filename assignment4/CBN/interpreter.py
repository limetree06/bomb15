...
def O(op, v, env):
    if(op == 'not') :
        ...
        if(len(v) != 2) : return err_to_comp (op + 'wrong-number-of-args')
            ...
            if op == 'unit=?':
                if (v[0] != '#u' | v[1] != '#u'| v[1]!='unit'|v[0]!='unit' ) : return err_to_comp ('not-an-unit')
                else : 
                    if v[0] == v[1] : return True
                    else : return False
    ...
    if type(a) == list :
        if type(value) == list:
            for i in range(len(value)):
                if type(value[i]) == int : return value[i]
...
def P(exp):
    ...
    else : return ValueError('NOT A FLK PROGRAM')
...
def E(exp, env):
    if type(exp) != list:
        ...
        if cmd == 'prim':
            ...
            results = []
            if cmd[0] == 'lam':
                if type(cmd[2]) == int : return cmd[2]
                elif type(cmd[2]) == bool : return cmd[2]
                elif type(cmd[2]) == tuple : return cmd[2]
